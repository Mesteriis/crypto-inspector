...
class AdaptiveNotificationsManager:
    ...
    async def _calculate_volatility_profile(self, symbol: str) -> VolatilityProfile:
        ...
        volume_change = (
            ((sum(volumes[-5:]) - sum(volumes[-10:-5])) / sum(volumes[-10:-5]) * 100) if len(volumes) >= 10 else 0
        )
    ...
    async def _check_condition(
        self, rule: AdaptiveNotificationRule, profile: VolatilityProfile, adjusted_threshold: float
    ) -> bool:
    ...
    async def _send_notification(
        self, rule: AdaptiveNotificationRule, profile: VolatilityProfile, timestamp: datetime
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            response = await http_client.post(
                "/core/api/services/persistent_notification/create", json=notification_data
            )
    ...
    def _format_notification_message(self, rule: AdaptiveNotificationRule, profile: VolatilityProfile) -> str:
    ...
    async def get_notification_stats(self) -> dict[str, Any]:
        ...
        if not self._notification_history:
            return {"total_sent": 0, "recent_24h": 0, "by_priority": {}, "by_symbol": {}}

        now = datetime.now()
        recent_cutoff = now - timedelta(hours=24)

        recent_notifications = [
            n for n in self._notification_history if datetime.fromisoformat(n["timestamp"]) > recent_cutoff
        ]
...
# Global instance
_adaptive_notifications: AdaptiveNotificationsManager | None = None
