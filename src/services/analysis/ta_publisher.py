...
class TAPublisher:
    ...
    async def _publish_to_ha(self, symbol: str, signals: TASignals) -> None:
        """Publish TA data to Home Assistant sensors using generic dictionary format."""
        if not self._mqtt:
            return
        ...
        try:
            ...
            # Publish aggregated dictionary sensors
            await self._publish_dict_sensor("ta_rsi", self._rsi_values)
            await self._publish_dict_sensor("ta_macd_signal", self._macd_signals)
            await self._publish_dict_sensor("ta_bb_position", self._bb_positions)
            await self._publish_dict_sensor("ta_trend", self._trends)
            await self._publish_dict_sensor("ta_support", self._supports)
            await self._publish_dict_sensor("ta_resistance", self._resistances)

            # Publish confluence (global metrics)
            await self._publish_sensor("ta_confluence", signals.confluence_score)
            await self._publish_sensor("ta_signal", signals.signal)

            logger.info(f"Published TA sensors for {symbol}")

        except Exception as e:
            logger.error(f"Failed to publish TA sensors: {e}")

    async def _publish_dict_sensor(self, sensor_id: str, data: dict) -> None:
        """Publish dictionary-based sensor value."""
        if self._mqtt and data:
            import json

            topic = f"crypto_inspect/{sensor_id}/state"
            await self._mqtt.publish(topic, json.dumps(data))
    ...
    async def _publish_sensor(self, sensor_id: str, value: Any) -> None:
        ...
        if self._mqtt and value is not None:
            topic = f"crypto_inspect/{sensor_id}/state"
            await self._mqtt.publish(topic, str(value))
