...
class DCABacktester:
    ...
    async def _publish_to_ha(
        ...
        """Publish backtest results to Home Assistant."""
        if not self._mqtt:
            return

        try:
            if "fixed_dca" in results and results["fixed_dca"]:
                await self._publish_sensor("backtest_dca_roi", results["fixed_dca"].total_return_pct)

            if "smart_dca" in results and results["smart_dca"]:
                await self._publish_sensor("backtest_smart_dca_roi", results["smart_dca"].total_return_pct)

            if "lump_sum" in results and results["lump_sum"]:
                await self._publish_sensor("backtest_lump_sum_roi", results["lump_sum"].total_return_pct)
            ...
            if best_strategy:
                ...
                await self._publish_sensor(
                    ...
                    strategy_names.get(best_strategy, best_strategy),
                )

            logger.info(f"Published backtest results - Best: {best_strategy}")

        except Exception as e:
            logger.error(f"Failed to publish backtest results: {e}")
