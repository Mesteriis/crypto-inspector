...
class AITrendAnalyzer:
    ...
    async def _analyze_ml_predictions(
        ...
        try:
            ...
            predictions_24h = await self.forecaster.predict(
                ...
                interval=interval,
                prices=prices,
                horizon=24 if interval == "1h" else 1
            )
            ...
            predictions_7d = await self.forecaster.predict(
                ...
                interval=interval,
                prices=prices,
                horizon=168 if interval == "1h" else 7
            )
    ...
    def _assess_risk_factors(
        ...
        if (market_context.get("volume_trend") == "decreasing" and 
            price_returns > 0):
    ...
    def _find_key_levels(self, prices: List[float]) -> tuple[List[float], List[float]]:
        ...
        for i in range(5, len(prices) - 5):
            ...
            if (prices[i] <= min(prices[i-5:i]) and 
                prices[i] <= min(prices[i+1:i+6])):
                support_levels.append(prices[i])
            
            # Check for local maximum (resistance)
            if (prices[i] >= max(prices[i-5:i]) and 
                prices[i] >= max(prices[i+1:i+6])):
    ...
    async def get_multiple_trends(
        ...
        tasks = [
            self.analyze_trend(symbol, interval)
...
# Global instance
_trend_analyzer: Optional[AITrendAnalyzer] = None
